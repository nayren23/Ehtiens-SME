...
def authenticate():
    ...
    try:
        ...
        response = make_response(jsonify(
            message="AUTHENTIFIED_SUCCESSFULLY", 
            pseudo_verified=user_bo.pseudo,
            is_admin=user_bo.is_admin
        ))
        user_id_str = str(user_bo.id)
        access_token = create_access_token(identity=user_id_str, additional_claims={"is_admin": user_bo.is_admin})
...
def logout():
    """Logout endpoint"""
    response = make_response(jsonify(message="LOGGED_OUT_SUCCESSFULLY"))
